...
async function handleSearch(ctx) {
  ...
  try {
    ...
    // Apply user's preferred format as default filter
    const prefs = getUserPrefs(ctx.from.id);
    const prefFormat = prefs.format || "epub";
    const filteredByPref = results.filter((r) => r.ext === prefFormat);
    const activeFormat = filteredByPref.length > 0 ? prefFormat : "all";
    state.results = activeFormat === "all" ? results : filteredByPref;

    // Check if preferred format is available
    if (activeFormat !== "all" && filteredByPref.length === 0) {
      ...
      return ctx.telegram.editMessageText(
        ...
        `📚 Pas de ${prefFormat.toUpperCase()} disponible pour « ${query} ».\n` +
    ...
    const filterRow = [
      Markup.button.callback(activeFormat === "all" ? "📚 ✓Tous" : "📚 Tous", "filter_all"),
      Markup.button.callback(activeFormat === "epub" ? "📗 ✓EPUB" : "📗 EPUB", "filter_epub"),
      Markup.button.callback(activeFormat === "pdf" ? "📕 ✓PDF" : "📕 PDF", "filter_pdf"),
      Markup.button.callback(activeFormat === "mobi" ? "📙 ✓MOBI" : "📙 MOBI", "filter_mobi"),
    ];
    const displayResults = state.results;
    const resultButtons = buildResultButtons(displayResults, 0);
